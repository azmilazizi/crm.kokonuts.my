...
class Wa_bill_drafts_model extends App_Model
{
    protected $table       = 'wa_bill_drafts';
    protected $attach_table = 'wa_bill_draft_attachments';
    ...
    public function get_drafts(array $filters, int $limit, int $offset): array
    {
        ...
        if (!empty($filters['search'])) {
            ...
            $this->db->or_like('d.reference_no', $s);
            $this->db->group_end();
        }
    ...
    public function get_draft(string $id): ?array
    {
        ...
        $row['bill_category_id'] = $row['bill_category_id'] ? (int) $row['bill_category_id'] : null;
