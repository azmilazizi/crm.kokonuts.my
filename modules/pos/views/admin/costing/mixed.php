...
// value (confirmed via console on the identical Product Cost Profit dropdown:
// first event reports the newly clicked item, a second event shortly after
// reports the old one again). If we see a different value within a short
// window of the one we just committed, treat it as that spurious echo and
// restore what the user actually picked instead of recomputing cost off the
// reverted value.
$('#mixedCostModal').on('change', '.mixed-component-item', function () {
    var $sel = $(this);
