...
class Migration_Version_136 extends App_module_migration
{
    public function up()
    {
        ...
        if ($CI->db->table_exists(db_prefix() . 'pos_item_yields')) {
            $cols = $CI->db->list_fields(db_prefix() . 'pos_item_yields');

            if (!in_array('reference_price', $cols)) {
                $CI->db->query("ALTER TABLE `" . db_prefix() . "pos_item_yields`
    ...
    public function down()
    {
        ...
        if ($CI->db->table_exists(db_prefix() . 'pos_item_yields')) {
            $cols = $CI->db->list_fields(db_prefix() . 'pos_item_yields');
            if (in_array('reference_price', $cols)) {
                $CI->db->query("ALTER TABLE `" . db_prefix() . "pos_item_yields` DROP COLUMN `reference_price`");
            }
        }
    }
}
