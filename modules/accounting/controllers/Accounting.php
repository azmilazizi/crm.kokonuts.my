...
class Accounting extends AdminController
{
    ...
    public function journal_entry_next_number()
    {
        ...
        $nextNumber  = $this->accounting_model->get_journal_entry_next_number($journalDate, false);
    ...
    public function import_xlsx_posted_bank_transactions() {
        ...
        $data['purchase_vendors'] = $this->db->select('userid,' . get_sql_select_vendor_company())
            ...
            ->get()
            ->result_array();
        $data['purchase_items'] = $this->db->select('id, description, long_description, rate, commodity_code')
            ->from(db_prefix() . 'items')
            ->where('can_be_purchased', 1)
            ->where('can_be_inventory', 1)
