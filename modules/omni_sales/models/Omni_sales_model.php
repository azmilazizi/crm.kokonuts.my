...
class Omni_sales_model extends App_Model
{
	...
	public function get_list_product_by_group($id_chanel, $id_group = '0', $id_warehouse = '', $key = '',$limit = 0, $ofset = 1){
		...
		$department = '';
		if(get_option('omni_show_products_by_department') == 1 && $id_chanel == 1){
	...
	public function check_out($data){
		...
		if($data_client){
			...
			$trade_discounts = $this->get_discount_list(2, get_client_user_id());
			...
			if($insert_id){
				...
				foreach ($productid_list as $key => $product_id) {
					...
					$discount_value = 0;
	                $price_af_dc = $prices;
	                 if(is_array($trade_discounts) && count($trade_discounts) > 0){
	                    foreach($trade_discounts as $discount_val){
	                      if($discount_val['minimum_order_value'] == 0){
	                        if($discount_val['formal'] == 1){
	                          $discount_value += $prices *  $discount_val['discount'] / 100;
	                         
	                        }else{
	                          $discount_value +=  $discount_val['discount'];
	                        }
	                      }
	                    }
	                }
	                $price_af_dc = $price_af_dc - $discount_value;

					$tax_array = [];
					$get_tax_data = $this->get_tax_list_product($product_id);
					if($get_tax_data){
						foreach ($get_tax_data as $tax) {
							$total_tax_value = ($tax['taxrate'] * ($price_af_dc * $item_quantity) / 100);
				...
				$status = 0;
				if(omni_get_status_modules('warehouse') && get_warehouse_option('auto_create_goods_delivery') == 1){
					$status = 2;
				}
				$this->add_inv_when_order($insert_id, $status);  
	...
	public function remove_cart_data_cookie(){
		if (isset($_COOKIE['cart_id_list'])&&isset($_COOKIE['cart_qty_list'])) {
			...
			setcookie('cart_id_list', '', -1, '/'); 
			setcookie('cart_qty_list', '', -1, '/'); 
			return true;
		} else {
			return false;
		}
	 ...
	 */
	public function get_cart($id = '', $where = '', $select = '*'){
	...
	public function process_orders_woo($store_id){
		...
		if($store && $woocommerce){
			...
			$order_number = [];
			$carts = $this->get_cart('','','order_number'); 
					...
					if(in_array($value->status, $status_allowed_to_sync) || count($status_allowed_to_sync) == 0){
						...
						if($value->customer_id > 0){
							...
							if(in_array($value->billing->country, $iso1)){
								...
								$client_data['country'] = $this->db->get('tblcountries')->row()->country_id;
							}
							$client_data['company'] = $first_name . ' ' . $last_name;
							$client_data['address'] = $street;
							$client_data['city'] = $city;
							$client_data['state'] = $state;
							$client_data['billing_street'] = $street;
							$client_data['billing_city'] = $city;
							$client_data['billing_state'] = $state;
							$client_data['billing_zip'] = $postcode;
							$client_data['billing_country'] = is_numeric($client_data['country']) ? $client_data['country'] : 0;
							$client_data['shipping_street'] = $street;
							$client_data['shipping_city'] = $city;
							$client_data['shipping_state'] = $client_data['country'];
							$client_data['shipping_zip'] = $postcode;
							$client_data['shipping_country'] = $client_data['country'];
							$client_data['country'] = $client_data['country'];
							$client_data['zip'] = $postcode;						

							$this->db->select('userid');
							$this->db->where('woo_customer_id', $value->customer_id);
							$this->db->where('woo_channel_id', $store_id);
							$this->db->where('active', '1');
							$client = $this->db->get(db_prefix().'clients')->row();
							if(!$client){
								$client_data['firstname'] = $first_name;
								$client_data['lastname'] = $last_name;
								$client_data['email'] = $value->billing->email;
								$client_data['contact_phonenumber'] = $value->billing->phone;
								$client_data['password'] = $password;
								$client_data['woo_customer_id'] = $value->customer_id;
								$client_data['woo_channel_id'] = $store_id;
								$this->clients_model->add($client_data, true);
							}
						}
						//
						if(!in_array($value->number, $order_number)){
							...
							$data_update['order_number'] =  $value->number;
							array_push($arr_update_batch, $data_update);
						}
					}
				}
				// Update cart data
				if(count($arr_update_batch)){
					$this->db->update_batch(db_prefix().'cart', $arr_update_batch, 'order_number');
				}
				return true;
			} 
		} 
	...
	public function add_order_woo($data, $woocommerce, $store_id, $store_name){
		...
		if($insert_id){
			...
			foreach ($productid_list as $key => $p_value) {
				...
				array_push($detail_insert_arr, $data_detailt);
			}
			$this->db->insert_batch(db_prefix() . 'cart_detailt', $detail_insert_arr);
			$this->remove_cart_data_cookie();
		}
		...
		if($cart_after_insert && is_object($cart_after_insert)){
			...
			if(isset($setting_inv)){
				if($setting_inv == 1){
					$id_invoice_rs = $this->add_inv_when_order_v2($insert_id, $cart_after_insert->status, $data['order_number']);
					if($id_invoice_rs){

						if($data['transaction_id'] != ''){
							$data_payment = [];
							$data_payment["invoiceid"] = $id_invoice_rs;
						 	$data_payment["amount"] = $data['total'];
						 	$data_payment["date"] = _d(date('Y-m-d'));
						 	$data_payment["paymentmode"] = $data['payment_mode'];
						 	$data_payment["do_not_redirect"] = 'off';
						 	$data_payment["transactionid"]=$data['transaction_id'];
						 	$data_payment["note"] = '';
	...
	*/
	...
	public function change_status_order($data, $order_number,$admin_action = 0){
		...
		if($data_order){
			...
			$this->db->update(db_prefix().'cart',$data_update);
			if ($this->db->affected_rows() > 0) {
            	hooks()->do_action('after_omni_sales_order_status_changed', ['data' => $data, 'data_order' => $data_order]);
	...
	public function add_inv_and_out_of_stock($orderid, $status = '') {
		...
		if($cart){
			...
			if(isset($cart->shipping) && (float)$cart->shipping > 0){
				if(get_option('omni_sale_hide_shipping_fee') == 0){
			...
			if($cart->discount_voucher > 0){
				$total = $cart->total + $cart->discount_voucher;
	 			$discount_total = $cart->discount_total;
			}
			...
			if($cart->discount_type == 1){
				$data['discount_percent' ]= $discount_total;
			...
			if($cart->discount_voucher != '' && $cart->discount_type != '' && $cart->voucher != ''){
				$credit_notes = $this->credit_note_from_invoice_omni_v2($id, $cart->voucher, $cart->discount_voucher);
			}            
			if($id){
...
public function get_discount($id = ''){
	...
	public function check_out_pos($data){
		...
		if($data_client){
			...
			$trade_discounts = $this->get_discount_list(1, $data['userid']);
	...
	public function credit_note_from_invoice_omni($invoice_id, $voucher)
	{
		...
		public function update_status_order_comfirm($order_id, $prefix = '' , $_invoice_number = '', $number = '', $status = 2){
			...
			if(omni_get_status_modules('warehouse')){
				$invoice_id = $this->get_id_invoice($number);
				if(is_numeric($invoice_id) && $invoice_id > 0){
					$this->db->where('invoice_id', $invoice_id);
					$goods_delivery = $this->db->get(db_prefix().'goods_delivery')->row();
					if($goods_delivery){
						$stock_export_number = $goods_delivery->id;
					}
				}
			}
			$code_invoice = $prefix . $_invoice_number;
			$this->db->where('id', $order_id);
			$dara = $this->db->update(db_prefix().'cart', ['status' => $status, 'admin_action' => 1, 'invoice' => $code_invoice, 'number_invoice' => $number, 'stock_export_number' => $stock_export_number]);
			if ($this->db->affected_rows() > 0) {
				return true;
			}
			return false;
		...
		public function add_inv_when_order($orderid, $status = '') {
			...
			if($cart){
				...
				if($cart->discount_type == 1){
					...
					$data['discount_total'] = $discount_total;
					if($data['subtotal'] > 0 && is_numeric($discount_total) && $discount_total > 0){
						$data['discount_percent'] =  ((float)$discount_total/(float)$data['subtotal'])*100;
					}
					else{
				...
				$this->load->model('omni_invoices_model');

				$id = $this->omni_invoices_model->add($data);
				if($cart->discount_voucher != '' && $cart->discount_type != '' && $cart->voucher != ''){
					$credit_notes = $this->credit_note_from_invoice_omni_v2($id, $cart->voucher, $cart->discount_voucher);
				} 
				$prefix = get_option('invoice_prefix');
	 ...
	 * @return bolean
	 */
		public function create_export_stock($orderid, $status = '', $invoice_id = '') {
			$this->load->model('warehouse/warehouse_model');
	...
	public function get_discount_list($channel_id, $client = '', $voucher=''){
		if($voucher == ''){
			if($client!=''){
				...
				$discount_list = $this->db->query($query)->result_array();      
	...
	*/
	public function sync_from_the_store_to_the_system($store_id, $omni_warehouse){
		...
		$products_all = $this->get_product('', '', 'sku_code, id');
		$store_name = $this->get_name_store($store_id);
		$array_sku_code = [];
		$array_id_by_sku = [];
		foreach ($products_all as $product) {
			if(!is_null($product['sku_code'])){
				array_push($array_sku_code, $product['sku_code']);
				$array_id_by_sku[$product['sku_code']] = $product['id'];
			}
		}
		...
		do{
			...
			foreach ($products_store as $key => $products) {
				...
				if(!in_array($products->sku, $array_sku_code)){
					...
					if($products->sku != ''){
						$sku = $products->sku;
						if(isset($array_id_by_sku[$sku]) && $item_id = $array_id_by_sku[$sku]){
							...
							$images_items = $this->get_image_id($item_id);
							...
							if(!empty($products->images)){
								foreach ($products->images as $image) {
									...
									$my_save_dir = 'modules/warehouse/uploads/item_img/'.$item_id.'/';
									...
									if(file_put_contents($complete_save_loc,file_get_contents($url_to_image, false, stream_context_create($arrContextOptions)))){
										...
										if(count($attachment) > 0){
											$this->misc_model->add_attachment_to_database($item_id, 'commodity_item_file', $attachment);
										}
									}
								}
							}
							...
							if($products->type == 'variable'){
								if(count($products->variations) > 0){
									foreach ($products->variations as $k_attr => $attribute) {
										...
										//init data variation
										$data_variation["commodity_code"] = $sku;
										...
										$data_variation["parent_id"] = $item_id;
	...
	*/
	public function test_connect($data)
	{
		...
		try {
			$woocommerce->get('products', ['per_page' => 1, 'offset' => 1000, 'page' => 1]);
	...
	*/
	...
	public function sync_from_the_system_to_the_store_single($store_id, $arr = null){
		...
		if(isset($arr)){
			...
			$products_list =  $this->get_list_product_store($store_id);
			foreach ($products_list as $key => $product) {
 ...
 */
	public function process_inventory_synchronization_detail($store_id, $arr_detail = null){
			...
			if($value->sku != ''){
				foreach ($items as $item) {
					if($item->sku_code == $value->sku){
						...
						$stock_quantity = $this->get_total_inventory_commodity($item->id);
						$stock_quantity = ($stock_quantity ? $stock_quantity->inventory_number : 0);
						$images = [];
						if($this->get_all_image_file_name($item->id)){
							$file_name = $this->omni_sales_model->get_all_image_file_name($item->id);
							foreach ($file_name as $k => $name) {
		...
		if(count($log_inventory) > 0){
			$this->db->insert_batch(db_prefix().'omni_log_sync_woo', $log_inventory);
		}
		return true;
	}
	/**
	...
	public function sync_all($store_id, $product_ids = []){
		...
		for($page = 1; $page <= 100; $page++ ){
			...
			foreach ($list_products as $value) {
				$products_store[$value->sku] = $value->id;
			}
		...
		$items = [];
	 	if($product_ids != []){
            ...
            for($page = 1; $page <= 100; $page++ ){
				...
				$this->db->select(db_prefix() . 'items.*, '.db_prefix().'items.id as id, '.db_prefix().'items_groups.id as group_id, '.db_prefix().'items_groups.name as group_name');
		 		$this->db->where(db_prefix().'items.id IN ('.$product_ids.')');
				$this->db->join(db_prefix() . 'items_groups', db_prefix() . 'items_groups.id = '.db_prefix() . 'items.group_id', 'left');
				$this->db->limit(1000, $offset);
		 		$list_items = $this->db->get(db_prefix().'items')->result_array();
				$items = array_merge($items, $list_items);
				if(count($list_items) < 1000){
					break;
				}
			}
	 	}else{
	 		$items = $this->get_list_product_store($store_id);
	 	}
		...
		for($page = 1; $page <= $this->per_page_tags; $page++ ){
			...
			foreach ($list_tags as $tag) {
				$list_tag[$tag->name] = ['id' => $tag->id, 'slug' => $tag->slug];
			}
		...
		for($page = 1; $page <= $this->per_page_product_categories; $page++ ){
			...
			foreach ($categories as $category) {
				$name = strtolower(html_entity_decode($category->name));
				$name = trim($name);
				$name = $this->vn_to_str($name);
				$name = $this->clean($name);
				$product_categories_list[$name] = $category->id;
			}
		...
		foreach ($items as $key => $value) {
			if(!is_null($value)){
				$count++;
				if(!isset($products_store[$value['sku_code']])){

					//get product images
					$images = [];
					$file_name = $this->get_all_image_file_name($value['id']);
					foreach ($file_name as $k => $name) {
						array_push($images, array('src' => site_url('modules/warehouse/uploads/item_img/'.$value['id'].'/'.$name['file_name'])));
					}
					...
					$date_on_sale_from = null;
					$date_on_sale_to = null;
					$discount =  $this->check_discount($value['id'], $date, 3, $store_id);
					if(!is_null($discount)){
						if($discount->formal == 1){
							$price_discount = $value['rate'] - (($value['rate'] * $discount->discount)/100);
						}else{
					...
					if(count($tags_name) > 0){
						$data_tag_ = [];
						foreach ($tags_name as $key_count => $t_name) {
							$tags_ = strtolower($t_name);
							$tags_ = trim($tags_);
							$tags_ = $this->vn_to_str($tags_);
							$name_tag = $this->clean($tags_);

							if(!isset($list_tag[$t_name])){
								$data_tag_[] = [
									'name' => $t_name 
								];
							}else{
								foreach ($list_tag as $name => $tag_id) {
									if($name == $t_name){
										$tags_final[] = ['id' => $tag_id]; 
									}
								}
							}
						...
						foreach ($data_tag_ as $data_1) {
							if(!isset($list_tag[$data_1["name"]])){
								$avbcs = $woocommerce->post('products/tags', $data_1);
								$list_tag[$data_1["name"]] = $avbcs->id;
								$tags_final[] = ['id' => $avbcs->id ];
							}
						}
					}
					...
					$categories = [];
					if($value['group_name'] != ''){
						$group_name = strtolower(html_entity_decode($value['group_name']));
						$group_name = trim($group_name);
						$group_name = $this->vn_to_str($group_name);
						$group_name = $this->clean($group_name);
						
						if(isset($product_categories_list[$group_name])){
							$categories[] = ['id' => $product_categories_list[$group_name]];
						}else{
							$category = $woocommerce->post('products/categories', ['name' => $value['group_name']]);
							$product_categories_list[$group_name] = $category->id;
					...
					array_push($data_create,$data);
					if(count($data_create) == $this->amount){
						$data_create_master[] = $data_create;
						$data_create = [];
					}
					$this->db->insert(db_prefix().'omni_log_sync_woo', $log_product);
				}else{
					...
					if(count($products_store) > 0){
						...
						$stock_quantity = $this->get_total_inventory_commodity($value['id']);
						if($this->get_all_image_file_name($value['id'])){
							$file_name = $this->get_all_image_file_name($value['id']);
						}
						...
						if(in_array($value['sku_code'], $products_variation['sku_code_s'])){
							...
							if(count($parent_attributes) > 0){
								$update_attr = $woocommerce->get('products/'.$products_store[$value['sku_code']]);
								foreach ($parent_attributes as $key_parent_attributes => $value_parent_attributes) {
						...
						$data_cus_update_2 = [
							'id' => $products_store[$value['sku_code']],
							'tags' => $tags_final,
							'type' => $type,
							'name' => $value['description'],
							'regular_price' => $regular_price_prices,
						...
						array_push($data_cus_update_, $data_cus_update_2);
						if(count($data_cus_update_) == $this->amount){
							$data_cus_update_master[] = $data_cus_update_;
							$data_cus_update_ = [];
						}
					}
				}
	...
	public function delete_order($id){
		...
		if($cart){
			$number_invoice = $cart->number_invoice;			
			$this->db->where('id',$id);
			...
			if ($this->db->affected_rows() > 0) {
				...
				// Delete invoice
				if(is_numeric($number_invoice) && $number_invoice > 0){
					$this->load->model('invoices_model');
					$invoice = $this->get_invoice($number_invoice);
					if($invoice){
						$this->invoices_model->delete($invoice->id, true);						
					}
				}
				//
				hooks()->do_action('after_omni_sales_order_deleted', $id);
				return true;
			}
		}
		return false;
...
public function get_max_version_omni_customer_report($next_version = false)
{
	...
	public function exit_type_variation($store_id, $status = 1){
			...
			if(count($list_products) > 0){
				foreach ($list_products as $key => $value) {
					if($value->type == "variable"){
						$products_store_variable[$value->sku] = $value->id;
					}
		...
		if(count($products_store_variable) > 0){
			foreach ($products_store_variable as $sku => $product_id) {
				$inventory_update = 0;
				if(in_array($sku, $products_variation['sku_code_s'])){
						//get id by index of array $products_variation['sku_code_s']
					$index = array_search($sku , $products_variation['sku_code_s'], true);
					...
					foreach ($variations as $key_variations => $value_variations) {
						...
						if($status == 1){
							...
							$variation_product_value = $woocommerce->get('products/'.$product_id.'/variations');
	 ...
	 * @return bolean
	 */
	public function add_inv_when_order_v2($orderid, $status = '', $woo_order_number = '') {
		...
		if($cart){
			...
			/*Invoices synced on Woocommerce will be set due date on the "X" day of every month.*/
			$omni_synch_invoice_from_woo = get_option('omni_synch_invoice_from_woo');
			if(strlen($omni_synch_invoice_from_woo) > 0){
				if(strlen($omni_synch_invoice_from_woo) == 2){
					$setting_date = $omni_synch_invoice_from_woo;
				}else{
					$setting_date = '0'.$omni_synch_invoice_from_woo;
				}
				$current_date = date("d");
				$due_data_temporaty =  date("Y-m-d", strtotime(date("Y").'-'.date("m").'-'.$setting_date));

				if((int)$current_date >= (int)$omni_synch_invoice_from_woo ){
					/*due date next month*/
					$data['duedate'] = _d(date("Y-m-d", strtotime("+1 month", strtotime($due_data_temporaty))));

				}else{
					$data['duedate'] = _d($due_data_temporaty);
				}
			}else{
				$data['duedate'] = date($date_format);
			}

			$data['woo_order_number'] = $woo_order_number;
			...
			if($cart->discount_type == 1){
				...
				$data['discount_total'] = ($data['subtotal'] > 0 ? $discount_total : '');
				$data['discount_percent'] =  ($data['subtotal'] > 0 ? (($discount_total/$data['subtotal'])*100) : '');
			}else{
				$data['discount_total'] = '';
				$data['discount_percent'] = '';
			}
	 ...
	 */
	 ...
	 /**
		 ...
		 * @return           
		 */
	 public function sync_short_description($store_id, $product_ids = []){
	 	$items = [];
	 	if($product_ids != []){
            $product_ids = implode(',', $product_ids);
	 		$this->db->where('id IN ('.$product_ids.')');
	 	...
	 	$store_name = $this->get_name_store($store_id);
		foreach ($items as $item) {
			if(isset($products_store[$item['sku_code']])){
				$data = [
					'id' => $products_store[$item['sku_code']],
					'short_description' => $item['long_description'],
				];

				array_push($data_update,$data);
				if(count($data_update) == $this->amount){
					$data_update_master[] = $data_update;
					$data_update = [];
				}
	 	...
	 	if(count($data_update_master) > 0){
	 		foreach ($data_update_master as  $data__) {
	 			$data_update_batch = [
	 				'update' => $data__
	 			];
	 			$woocommerce->post('products/batch', $data_update_batch);
	 		}
	 		$this->exit_type_variation($store_id, 2);

	 		if(count($logs) > 0){
				$this->db->insert_batch(db_prefix().'omni_log_sync_woo', $logs);
			}
	 	}
	 	return true;
	 }
	...
	public function omnisales_auto_create_goods_delivery_with_invoice($invoice_id, $invoice_update='')
	{
		...
		public function omnisalse_add_goods_delivery_from_invoice($data_insert, $invoice_id ='')
		{
			...
			//update approval
				...
				foreach ($goods_delivery_detail as $goods_delivery_detail_value) {
					// Remove This because sub duplicate Quantity 
					...
					$goods_delivery_detail_value['purchase_price'] = '0';
   ...
   */
   public function create_new_order($data){
   	...
   	if($data_client){
   		...
   		$data_cart['discount_type_str'] = $discount_type;
   		// $data_cart['discount_total'] = $data['discount_total'];
   		...
   		if($insert_id){
   			...
   			foreach ($newitems as $key => $value) {
				...
				//Save tax to json
   				$rate_for_tax = $value['rate'];
   				if($discount_type == 'before_tax'){
   					$rate_for_tax = $value['rate'] - $prices_discount;
   				}
   				...
   				if(is_numeric($data_cart['estimate_id'])){
   					...
   					if($get_tax_data){
   						foreach ($get_tax_data as $tax) {
   							$total_tax_value = ($tax['taxrate'] * ($rate_for_tax * $item_quantity) / 100);
   							$total_tax += $total_tax_value;
   							$tax_array[] = [
   								'id' => $tax['id'],
   								'name' => $tax['name'],
...
public function has_variation($parent_attributes){
	$has_classify = false;
	$classify_list = omni_json_decode($parent_attributes);
		...
		$data_cart['shipping'] =  $data['shipping'];
  		...
  		if($insert_id){
  			...
  			foreach ($data['newitems'] as $key => $value) {
  				...
  				$discount_value = 0;
                $price_af_dc = $prices;
                if(is_array($list_discount) && count($list_discount) > 0){
                    foreach($list_discount as $discount_val){
                      if($discount_val['minimum_order_value'] == 0){
                        if($discount_val['formal'] == 1){
                          $discount_value += $prices *  $discount_val['discount'] / 100;
...
public function create_purchase_request($id){
	...
	if($data_cart){
		...
		if($data_detail){
			...
			$purchase_data['department'] =  $data['department'];
			$data_base_currency = $this->currencies_model->get_base_currency();
			if($data_base_currency){
				$purchase_data['currency'] =  $data_base_currency->id;
			}
			else{
				$purchase_data['currency'] =  0;
			}
			$purchase_data['newitems'] = $data_dt;
          	//add purchase request
			$pur_request_id = $this->purchase_model->add_pur_request($purchase_data);
			return $pur_request_id;
		}
...
public function create_invoice_detail_order($orderid, $status = '') {
	...
	if($cart){
		...
		if($cart->discount_voucher > 0){
			$total = $cart->total + $cart->discount_voucher;
 			$discount = $cart->discount - $cart->discount_voucher;
		}
		...
		$data['shipping_state'] = $cart->shipping_state;
		$data['shipping_zip'] = $cart->shipping_zip;
		if(isset($cart->shipping) && (float)$cart->shipping_value > 0){
			$data['shipping_fee'] = $cart->shipping_value;
			if(get_option('omni_sale_hide_shipping_fee') == 0){
				array_push($newitems, array('order' => $count+1, 'description' => _l('shipping'), 'long_description' => "", 'qty' => 1, 'unit' => "", 'rate'=> $cart->shipping_value, 'taxname' => array()));
			}
		}
		else if(isset($cart->shipping) && (float)$cart->shipping > 0){
			$data['shipping_fee'] = $cart->shipping;
		...
		if($cart->channel_id == 3){
			...
			$data['woo_order_number'] = $cart->order_number;
		}else{
			$data['duedate'] = date($date_format);
		}
	...
	public function get_discount_item_portal($item_id, $client = '', $date = ''){
		...
		if($data_group_product){
			...
			if(is_numeric($group_product) && $group_product > 0){
				...
				$group_product_query = ' and (group_items = "" or group_items is null)';
			}
		}
	...
	public function omni_sale_detail_order_return($id) {
		...
		if($cart_data){
			...
			foreach ($cart_detail_data as $key => $row) {	
				...
				$unit_id = '';
				$commodity_code = '';
				$item = $this->omni_sales_model->get_product($row['product_id']);
				if($item){
					$commodity_code = $row['product_id'];
					if($item->unit_id){
...
/**
	...
	public function change_status_return_order($data){
	  // Status change to canceled
		...
		if ($this->db->affected_rows() > 0) {
			$cart = $this->get_cart($data['order_id']);
			if($data['status'] == 1){
				$this->db->where('rel_id', $cart->original_order_id);
	...
	*/
	...
	public function update_invoice($order_id, $invoice_id){
		...
		if($invoice_data && $cart_data){
			...
			$invoice_data_update['billing_street'] = $invoice_data->billing_street;
 ...
 /**
    ...
    public function create_refund($data)
    {
        ...
        hooks()->do_action('after_omni_sales_refund_added', $insert_id);
    ...
    public function edit_refund($data)
    {
        ...
        if ($this->db->affected_rows() > 0) {
        	hooks()->do_action('after_omni_sales_refund_updated', $data['id']);
        	return true;
        }
        return false;
    }
    ...
    public function delete_refund($id){
		...
		if ($this->db->affected_rows() > 0) {           
        	hooks()->do_action('after_omni_sales_refund_deleted', $id);
			return true;
		}
		return false;
    }
	...
	public function order_list_get_estimate_data($invoice_id)
	{
		...
		if($estimates_value){
			...
			'.render_input('unit_name', '', _l('unit'), 'text', ['placeholder' => _l('unit'), 'readonly' => true], [], 'no-margin', 'input-transparent text-right wh_input_none').'
    ...
    /**
	 * delete order after delete invoice
	 * @param  integer $invoice_id 
	 * @return boolean             
	 */
    public function delete_order_after_delete_invoice($invoice_id){
    	$this->db->where('id', $invoice_id);
    	$invoice = $this->db->get(db_prefix().'invoices')->row();
    	if($invoice){
    		$this->db->where('number_invoice', $invoice->number);
    		$cart = $this->db->get(db_prefix().'cart')->result_array();
    		if(is_array($cart)){
    			foreach ($cart as $key => $value) {
    				$id = $value['id'];
    				$this->db->where('id', $id);
    				$this->db->update(db_prefix().'cart', ['invoice' => '', 'number_invoice' => '', 'stock_export_number' => '']);
    			}
    		}
    	}
    }
	 ...
	 * after invoice added
	 */
	public function after_invoice_added($data)
	{
		if (isset($data['order_id']) && $data['order_id'] != '' && $order_id = $data['order_id']) {
			if (omni_get_status_modules('warehouse') && get_warehouse_option('auto_create_goods_delivery') == 1) {
				$this->db->select('channel');
				$this->db->where('id', $order_id);
				$cart_data = $this->db->get(db_prefix() . 'cart')->row();
				if ($cart_data && $cart_data->channel != 'pos') {
					//purchase order is approval
					$this->create_export_stock($order_id, 2, $data['invoice_id']);
				}
			}
		}
	}
	 ...
	 * @return object          
	 */
	public function get_discount_portal($item_id, $price, $user_id, $date){
		$discount_percent = 0;
		$prices_discount  = 0;
		$discount = $this->get_discount_item_portal($item_id, $user_id, $date);
		...
		if($discount){
			if($discount->formal == 1){
				...
				if($price > $discount_amount){
					$prices_discount = round((float)$price-(float)$discount_amount, 2);
					$discount_percent = (($price > 0) ? ($discount_amount * 100 / $price) : 0);
				}else{
					$prices_discount = 0;
					$discount_percent = 0;
				}
		...
		$obj = new stdClass();
		$obj->percent = $discount_percent;
		$obj->price = $prices_discount;
		return $obj;
	}

	/**
	 * get list product store
	 * @param string $store_id 
	 * @return array        
	 */
	public function get_list_product_store($store_id = ''){
		$this->db->select(db_prefix() . 'ware_unit_type.unit_name'.','.db_prefix() . 'items.*, '.db_prefix().'items_groups.id as group_id, '.db_prefix().'items_groups.name as group_name');
		$this->db->where(db_prefix() . 'woocommere_store_detailt.woocommere_store_id', $store_id);
		$this->db->join(db_prefix() . 'ware_unit_type', db_prefix() . 'ware_unit_type.unit_type_id=' . db_prefix() . 'items.unit_id', 'left');
		$this->db->join(db_prefix() . 'woocommere_store_detailt', db_prefix() . 'woocommere_store_detailt.product_id=' . db_prefix() . 'items.id', 'left');
		$this->db->join(db_prefix() . 'items_groups', db_prefix() . 'items_groups.id = '.db_prefix() . 'items.group_id', 'left');
		return $this->db->get(db_prefix().'items')->result_array();
	}

	public function get_discount_item($item_id, $client = '', $date = ''){
		$group_product_query = '';
		$this->db->select('group_id');
		$this->db->where('id',$item_id);
		$data_group_product =  $this->db->get(db_prefix().'items')->row();
	...
	public function credit_note_from_invoice_omni_v2($invoice_id, $voucher, $discount_voucher)
	{
		$this->load->model('invoices_model');
		$this->load->model('credit_notes_model');
		$_invoice = $this->invoices_model->get($invoice_id);
		...
		$new_credit_note_data['show_quantity_as'] = $_invoice->show_quantity_as;
		$new_credit_note_data['currency']         = $_invoice->currency;
		$new_credit_note_data['subtotal']         = $discount_voucher;
		$new_credit_note_data['total']            = $discount_voucher;
		$new_credit_note_data['adminnote']        = $_invoice->adminnote;
		...
		$new_credit_note_data['newitems'][$key]['rate']  = $discount_voucher;
		$new_credit_note_data['newitems'][$key]['order'] = 1;
		...
		if ($id) {
			if ($_invoice->status != 2) {
				if ($this->credit_notes_model->apply_credits($id, ['invoice_id' => $invoice_id, 'amount' => $discount_voucher])) {
					update_invoice_status($invoice_id, true);
				}
			}
