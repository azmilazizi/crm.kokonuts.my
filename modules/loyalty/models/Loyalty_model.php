...
class Loyalty_model extends App_Model
{
    ...
    public function get_franchisees($active_only = false)
    {
        $this->db->select('f.*,
            ...
            ->from(db_prefix() . 'pos_loyalty_franchisees f');
        if ($active_only) {
    ...
    {
        return $this->db->get_where(db_prefix() . 'pos_loyalty_franchisees', ['id' => (int)$id])->row_array();
    }

    public function create_franchisee($data)
    {
        $data['created_at'] = date('Y-m-d H:i:s');
        $this->db->insert(db_prefix() . 'pos_loyalty_franchisees', $data);
        return $this->db->insert_id();
    }

    public function update_franchisee($id, $data)
    {
        $this->db->where('id', (int)$id)->update(db_prefix() . 'pos_loyalty_franchisees', $data);
        return $this->db->affected_rows() > 0;
    }
    ...
    public function delete_franchisee($id)
    {
        ...
        $this->db->where('id', $id)->delete(db_prefix() . 'pos_loyalty_franchisees');
        return true;
    }
    ...
    public function get_stores_with_franchisee()
    {
        return $this->db
            ...
            ->join(db_prefix() . 'pos_loyalty_franchisees f', 'f.id = w.franchisee_id', 'left')
            ->where('w.display', 1)
    ...
    public function get_franchisees_summary()
    {
        ...
        foreach ($franchisees as &$f) {
            ...
            $f['lifetime_transferred'] = (float)($this->db->select('SUM(amount) as s')
                ...
                ->get(db_prefix() . 'pos_loyalty_franchise_transfers')->row()->s ?? 0);
        }
        return $franchisees;
    }
    ...
    public function record_franchise_transfer($franchisee_id, $data)
    {
        ...
        $this->db->insert(db_prefix() . 'pos_loyalty_franchise_transfers', [
    ...
    public function delete_franchise_transfer($id)
    {
        ...
        $this->db->where('id', $id)->delete(db_prefix() . 'pos_loyalty_franchise_transfers');
        $this->db->trans_complete();
        return $this->db->trans_status() !== false;
    }
    ...
    public function count_franchise_transfers($franchisee_id)
    {
        return (int)$this->db->where('franchisee_id', (int)$franchisee_id)
            ->count_all_results(db_prefix() . 'pos_loyalty_franchise_transfers');
    }
    ...
    public function get_franchise_transfers($franchisee_id, $page = 1, $per_page = 20)
    {
        ...
        return $this->db->select('t.*, s.firstname, s.lastname')
            ->from(db_prefix() . 'pos_loyalty_franchise_transfers t')
