...
class Migration_Loyalty_franchise_schema extends App_module_migration
{
    public function up()
    {
        $CI = &get_instance();

        if (!$CI->db->table_exists(db_prefix() . 'pos_loyalty_franchisees')) {
            $CI->db->query('CREATE TABLE `' . db_prefix() . "pos_loyalty_franchisees` (
        ...
        if (!$CI->db->field_exists('franchise_transfer_id', db_prefix() . 'pos_loyalty_transactions')) {
            ...
            // (the API never passed it through) — recover it from the linked receipt so
        ...
        if (!$CI->db->table_exists(db_prefix() . 'pos_loyalty_franchise_transfers')) {
            $CI->db->query('CREATE TABLE `' . db_prefix() . "pos_loyalty_franchise_transfers` (
                `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    public function down()
    {
        ...
        if ($CI->db->table_exists(db_prefix() . 'pos_loyalty_franchise_transfers')) {
            $CI->db->query('DROP TABLE `' . db_prefix() . 'pos_loyalty_franchise_transfers`');
        }
        if ($CI->db->field_exists('franchise_transfer_id', db_prefix() . 'pos_loyalty_transactions')) {
            $CI->db->query('ALTER TABLE `' . db_prefix() . "pos_loyalty_transactions` DROP COLUMN `franchise_transfer_id`");
        }
        if ($CI->db->field_exists('franchisee_id', db_prefix() . 'warehouse')) {
            $CI->db->query('ALTER TABLE `' . db_prefix() . "warehouse` DROP COLUMN `franchisee_id`");
        }
        if ($CI->db->table_exists(db_prefix() . 'pos_loyalty_franchisees')) {
            $CI->db->query('DROP TABLE `' . db_prefix() . 'pos_loyalty_franchisees`');
        }
    }
}
