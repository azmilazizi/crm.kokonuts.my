...
class Loyalty extends AdminController
{
    ...
    public function ai_chat()
    {
        ...
        $this->load->view('loyalty/admin/ai_chat', $data);
    }
    ...
    public function ajax_ai_chat()
    {
        ...
        // Build Gemini contents from visible history (user/model text turns only)
        $contents = [];
        ...
        // Run tool-call loop (max 6 iterations to handle chained calls)
        $tool_calls  = [];
        $final_text  = null;
        $max_iter    = 6;
        ...
        for ($i = 0; $i < $max_iter; $i++) {
            ...
            if (isset($resp['error'])) {
                echo json_encode(['success' => false, 'error' => $resp['error']['message'] ?? 'Gemini API error']);
                return;
            }
    ...
    private function _execute_ai_tool($name, $args)
    {
        ...
        switch ($name) {
            ...
            case 'get_sales_trend':
                ...
                return array_slice($rows, 0, 60); // cap rows to limit tokens
