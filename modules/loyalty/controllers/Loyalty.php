...
class Loyalty extends AdminController
{
    ...
    public function franchise()
    {
        ...
        $data['franchisees'] = $this->loyalty_model->get_franchisees_summary();
        $data['stores']      = $this->loyalty_model->get_stores_with_franchisee();

        $this->load->view('loyalty/admin/franchise', $data);
    }
    ...
    public function franchisee($id = 0)
    {
        ...
        $franchisee = $this->loyalty_model->get_franchisee((int)$id);
        if (!$franchisee) {
            show_404();
        }
        ...
        $data['outstanding'] = $this->loyalty_model->get_franchisee_outstanding((int)$id);
        $data['outlets']     = array_filter($this->loyalty_model->get_stores_with_franchisee(), function ($s) use ($id) {
            return (int)($s['franchisee_id'] ?? 0) === (int)$id;
        });
        $data['redemptions'] = $this->loyalty_model->get_franchisee_redeem_transactions((int)$id, false, 1, 10);
        $data['transfers']   = $this->loyalty_model->get_franchise_transfers((int)$id, $page, $per_page);
        $transfer_total      = $this->loyalty_model->count_franchise_transfers((int)$id);
        $data['result']      = [
        ...
        $this->load->view('loyalty/admin/franchisee_detail', $data);
    }
    ...
    public function ajax_save_franchisee()
    {
        ...
        if ($id) {
            $this->loyalty_model->update_franchisee($id, $fields);
        } else {
            $id = $this->loyalty_model->create_franchisee($fields);
        }
    ...
    public function ajax_delete_franchisee($id)
    {
        ...
        $ok = $this->loyalty_model->delete_franchisee((int)$id);
        echo json_encode($ok
            ? ['success' => true]
    ...
    public function ajax_assign_store()
    {
        ...
        $this->loyalty_model->set_store_franchisee($warehouse_id, $franchisee_id);
        echo json_encode(['success' => true]);
    }
    ...
    public function ajax_record_transfer($franchisee_id)
    {
        ...
        $outstanding   = $this->loyalty_model->get_franchisee_outstanding($franchisee_id);
        ...
        $transfer_id = $this->loyalty_model->record_franchise_transfer($franchisee_id, [
            'amount'         => $outstanding,
    ...
    public function ajax_delete_transfer($id)
    {
        ...
        $ok = $this->loyalty_model->delete_franchise_transfer((int)$id);
