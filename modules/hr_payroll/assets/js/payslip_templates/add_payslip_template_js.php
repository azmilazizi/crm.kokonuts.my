<script >
	function normalizeSheet(sheet) {
		...
		if (!sheet.data || !Array.isArray(sheet.data) || sheet.data.length === 0) {
			const rows = sheet.row || 36;
			const cols = sheet.column || 26;
			sheet.data = Array.from({ length: rows }, () => Array(cols).fill(null));
		}

		// Fix missing celldata
		if (!sheet.celldata || sheet.celldata.length === 0) {
			sheet.celldata = luckysheet.transToCellData(sheet.data);
		}

		// Fix row and column
		sheet.row = sheet.data.length;
		sheet.column = sheet.data[0]?.length || 26;
		...
		if (sheet.config.merge) {
			...
			if (!m || m.r == null || m.c == null || m.rs == null || m.cs == null) {
	...
	(function(){
		...
		dataSheet = dataSheet.map(sheet => {
			// rebuild .data from celldata if missing
			if ((!sheet.data || sheet.data.length === 0) && sheet.celldata) {
				sheet.data = luckysheet.transToData(sheet.celldata);
			}
	...
	$(".luckysheet_info_detail_save").unbind('click').on('click', function(){
		$("form#spreadsheet-test-form").unbind('submit').on('submit', function(e) {
			...
			const rawData = luckysheet.getAllSheets();  // ← the most complete, reliable method
