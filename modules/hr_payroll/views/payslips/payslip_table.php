	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if($aColumns[$i] == 'id') {
			...
			if((has_permission('hrp_payslip','','delete')) && $aRow['payslip_status'] == 'payslip_closing' ){
				...
				$_data = '<a class="btn btn-secondary btn-xs mleft5" id="confirmDelete" data-toggle="tooltip" title="" href="'. admin_url('hr_payroll/payslip_update_status/'.$aRow['id']).'"  data-original-title="'._l('payslip_opening').'"><i class="fa fa-check"></i></a>';
