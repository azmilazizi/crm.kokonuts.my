        ...
        if(c  >= 2){
          ...
          for(let i = 0; i < array_route_point.length; i++){
            ...
            if(not_to_be_in_order == true){
              $('#route select[name="route_point[]"]:eq('+i+')').val(select_value.trim()).change();
            }
            else{
              // with order
              var obj = get_content_between_character(select_value, '(', ')');              
              $('#route select[name="route_point[]"]:eq('+i+')').val(obj.remain.trim()).change();
            }
          }
          if(not_to_be_in_order == true){
