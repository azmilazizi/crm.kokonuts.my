...
abstract class API_Controller extends CI_Controller {
    ...
    /**
     * Optional path to a module specific config directory.
     * When provided, the REST controller will merge the configuration
     * located in this directory with the global application configuration.
     *
     * @var string|null
     */
    protected $module_config_path = NULL;

    /**
     * Optional language file to load for API responses.
     * When null no additional language file is loaded automatically.
     *
     * @var string|null
     */
    protected $module_language_file = NULL;

    /**
     * Base directory for the optional language file.
     *
     * @var string|null
     */
    protected $module_language_directory = NULL;
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->module_language_file !== NULL)
        {
            ...
            {
                $language_directory = APPPATH . 'language/';
            }

            $this->lang->load($this->module_language_file, $language, FALSE, TRUE, $language_directory);
        }
    ...
    {
        if($this->module_config_path !== NULL)
        {
            $config_path = rtrim($this->module_config_path, '/').'/';

            if(file_exists($config_path.$config_file.'.php'))
            {
                $config = [];
                include($config_path.$config_file.'.php');
    ...
    /**
     * Validate the request authorization token.
     * Sends a HTTP 401 response when the token is missing or invalid.
     *
     * @return array|bool Token payload when valid, otherwise FALSE.
     */
    protected function authenticate_token()
    {
        $is_valid_token = $this->authorization_token->validateToken();

        if(isset($is_valid_token['status']) && $is_valid_token['status'] === FALSE)
        {
            $message = [
                'status' => FALSE,
                'message' => isset($is_valid_token['message']) ? $is_valid_token['message'] : 'Unauthorized request',
            ];

            $this->response($message, self::HTTP_UNAUTHORIZED);
